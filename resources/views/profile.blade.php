    ...
    <div class="profile-page grid min-h-screen grid-cols-1 md:grid-cols-[280px_1fr]">
        <aside class="hidden md:block sticky card top-0 h-screen bg-[#f7f7f8] p-6 border-r border-gray-200">
            <nav class="space-y-2">
                ...
                <a href="#replies" class="block p-2 hover:bg-white rounded">
                    <div class="flex flex-row gap-2">
                        <x-ts-icon name="chat-bubble-left-right" class="size-6" />
                        Replies
                    </div>
                </a>
                <a href="#follow" class="block p-2 hover:bg-white rounded">
                    <div class="flex flex-row gap-2">
                        <x-ts-icon name="user-plus" class="size-6" />
                        Follow
                    </div>
                </a>
                <a href="#block" class="block p-2 hover:bg-white rounded">
                    <div class="flex flex-row gap-2">
        ...
        <div class="p-8">
            ...
            <div>
                <div><a class="flex flex-row w-fit gap-2 !text-gray-500 items-center text-sm -mt-6 mb-8 hover:!text-blue-600 transition-colors duration-500 ease-in-out"
                        href="/profile/{{ $user->id }}"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"
                            fill="currentColor" class="size-[1em]">
                            <path fill-rule="evenodd"
            ...
            <h2 id="profile" class="scroll-mt-20 text-2xl font-bold mt-4">Your Profile</h2>
                            ...
                            @if (!auth()->user()->hasVerifiedEmail())
                                <div class="flex flex-col md:flex-row gap-2 items-center">
                                    ...
                                    <button type="button"
                                        ...
                                        class="justify-self-end btn btn-ghost btn-sm text-primary">
                                        Verify Email
                                    </button>
                                </div>
                            @else
                                <span class="flex gap-1"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"
                                        fill="currentColor"
                                        class="size-6 hover:text-[#4A40A9] hover:scale-110 origin-center text-[#4697E7] transition-all duration-200 ease-in-out mt-0.5 flex-shrink-0">
                                        <path fill-rule="evenodd"
                                    ...
                                    {{ $user->email_verified_at?->format('F d, Y') }}</span>
                            @endif
                        </div>
                </div>
            ...
            <div class="flex flex-col gap-y-2">
                ...
                @endforelse
                {{ $chirps->withQueryString()->onEachSide(1)->fragment('chirps')->links() }}
            </div>

            <h2 id="replies" class="scroll-mt-20 text-2xl font-bold mt-12 mb-2">Your Replies</h2>
            <hr class="border-gray-300 mb-4" />
            <div class="flex flex-col gap-y-2">
                @forelse ($replies as $reply)
                    <x-chirp :chirp="$reply" />
                @empty
                    <p class="text-gray-500 text-lg">No replies have been created yet.</p>
                @endforelse
            ...
            <div id="follow" class="flex flex-1 flex-col md:flex-row gap-8 scroll-mt-20 mt-20">
                <div class="flex-1">
                    ...
                    <div class="flex flex-col gap-2">
                                    ...
                                    <div class="flex space-x-3">
                                        ...
                                        <div onclick="event.stopPropagation()">
                                            <x-ts-dropdown icon="ellipsis-horizontal" class="z-20" static>
                                                <livewire:follow :user="$follow" />
                                                <livewire:block :user="$follow" />
                                            </x-ts-dropdown>
                                        </div>
                                    </div>
                                </div>
                        ...
                        @endforelse
                        {{ $follows->withQueryString()->onEachSide(1)->fragment('follow')->links() }}

                    </div>
                </div>

                <div class="flex-1">
                    <h2 class="text-2xl font-bold mb-2">Your Followed By</h2>
                    <hr class="border-gray-300 mb-4" />
                    <div class="flex flex-col gap-2">
                        ...
                        @empty
                            <p class="text-gray-500 text-lg">No users are following you yet.</p>
                        @endforelse
                        {{ $followers->withQueryString()->onEachSide(1)->fragment('follow')->links() }}

                    </div>
                </div>
            </div>
            <h2 id="block" class="text-2xl font-bold mt-12 mb-2 scroll-mt-20">Your Blocking</h2>
            <hr class="border-gray-300 mb-4" />
            <div class="flex flex-col gap-2">
                @forelse ($blocks as $block)
                    <div class="card bg-base-100 cursor-pointer relative hover:!bg-gray-200 !transition-colors !duration-500 !ease-in-out"
                        onclick="if(!window.getSelection().toString()) { Livewire.navigate('/profile/{{ $block->id }}') }">
                        <div class="card-body">
                            <div class="flex space-x-3">
                                @php $profileUrl = $block->name ? "/profile/{$block->id}" : "#"; @endphp

                                <div class="avatar relative z-20">
                                    <a href="{{ $profileUrl }}" onclick="event.stopPropagation()">
                                        <div class="size-10 rounded-full">
                                            <img loading="lazy" src="{{ $block->avatar_url }}"
                                                alt="{{ $block->name }}'s avatar" class="rounded-full" />
                                        </div>
                                    </a>
                                </div>
                                <div class="min-w-0 flex-1">
                                ...
                                <div onclick="event.stopPropagation()">
                                    <x-ts-dropdown icon="ellipsis-horizontal" class="z-20" static>
                                        <livewire:follow :user="$block" />
                                        <livewire:block :user="$block" />
                                    </x-ts-dropdown>
                                </div>
                            </div>
                        </div>
                ...
                @empty
                    <p class="text-gray-500 text-lg">No users have been blocked yet.</p>
                @endforelse
                {{ $blocks->withQueryString()->onEachSide(1)->fragment('block')->links() }}
                ...
                <h2 id="settings" class="scroll-mt-20 text-2xl font-bold mt-12 mb-2">Your Account</h2>
                <hr class="border-gray-300 mb-4" />
                <form method="POST" action="/logout" class="inline">
                    @csrf
                    <button type="submit"
                        class="btn btn-ghost !bg-white hover:!bg-gray-300 w-56 !p-6 !text-base"><x-ts-icon
                            icon="arrow-left-on-rectangle" class="size-6" /> Logout</button>
                </form>
                <form action="/settings/delete/{{ $user->id }}" method="POST">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
                        class="btn hover:bg-red-500 bg-white mt-2 hover:text-white text-red-500 w-56 !p-6"
        ...
        <form id="verify-form" method="POST" action="{{ route('verification.send') }}" class="hidden">
            @csrf
        </form>
